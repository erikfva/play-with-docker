...
async function withCredentialDir(fn) {
  ...
  const previousEnv = {
    ...
    CODESANDBOX_KEEP_ALIVE_ENABLED: process.env.CODESANDBOX_KEEP_ALIVE_ENABLED,
    CODESANDBOX_KEEP_ALIVE_INTERVAL_MINUTES: process.env.CODESANDBOX_KEEP_ALIVE_INTERVAL_MINUTES,
    CODESANDBOX_HIBERNATION_TIMEOUT_SECONDS: process.env.CODESANDBOX_HIBERNATION_TIMEOUT_SECONDS,
  ...
  delete process.env.CODESANDBOX_KEEP_ALIVE_ENABLED;
  delete process.env.CODESANDBOX_KEEP_ALIVE_INTERVAL_MINUTES;
  delete process.env.CODESANDBOX_HIBERNATION_TIMEOUT_SECONDS;
...
test('stores CodeSandbox keep-alive configuration at session creation time', async () => {
  await withCredentialDir(async (dir) => {
    const createdOptions = [];
    const dbPath = require.resolve('../src/db/db');
    const clientPath = require.resolve('../src/services/providers/codesandbox/client');
    ...
    assert.equal(createdOptions[0].hibernationTimeoutSeconds, 86400);
    assert.deepEqual(provider.getKeepAliveConfig({ metadata: session.metadata }), {
      enabled: true,
      intervalMinutes: 60,
      strategy: 'codesandbox-sdk-command',
      runOnStart: true
    });
    assert.equal(session.metadata.keepAlive.enabled, true);
    assert.equal(session.metadata.keepAlive.intervalMinutes, 60);
    assert.equal(session.metadata.keepAlive.hibernationTimeoutSeconds, 86400);
    assert.equal(session.metadata.keepAlive.strategy, 'codesandbox-sdk-command');
    assert.match(session.metadata.keepAlive.createdAt, /^\d{4}-\d{2}-\d{2}T/);
  });
});

test('uses CodeSandbox keep-alive and hibernation defaults from environment', async () => {
  await withCredentialDir(async (dir) => {
    const createdOptions = [];
    const dbPath = require.resolve('../src/db/db');
    const clientPath = require.resolve('../src/services/providers/codesandbox/client');
    ...
    process.env.CODESANDBOX_KEEP_ALIVE_ENABLED = 'true';
    process.env.CODESANDBOX_KEEP_ALIVE_INTERVAL_MINUTES = '45';
    process.env.CODESANDBOX_HIBERNATION_TIMEOUT_SECONDS = '7200';
    ...
    assert.equal(createdOptions[0].hibernationTimeoutSeconds, 7200);
    assert.equal(session.metadata.keepAlive.enabled, true);
    assert.equal(session.metadata.keepAlive.intervalMinutes, 45);
    assert.equal(session.metadata.keepAlive.hibernationTimeoutSeconds, 7200);
  });
});

test('request CodeSandbox keep-alive values override environment defaults', async () => {
  await withCredentialDir(async (dir) => {
    const createdOptions = [];
    const dbPath = require.resolve('../src/db/db');
    const clientPath = require.resolve('../src/services/providers/codesandbox/client');
    ...
    process.env.CODESANDBOX_KEEP_ALIVE_ENABLED = 'false';
    process.env.CODESANDBOX_KEEP_ALIVE_INTERVAL_MINUTES = '45';
    process.env.CODESANDBOX_HIBERNATION_TIMEOUT_SECONDS = '7200';
    ...
    assert.equal(createdOptions[0].hibernationTimeoutSeconds, 3600);
    assert.equal(session.metadata.keepAlive.enabled, true);
    assert.equal(session.metadata.keepAlive.intervalMinutes, 30);
    assert.equal(session.metadata.keepAlive.hibernationTimeoutSeconds, 3600);
  });
});
...
test('executes CodeSandbox keep-alive through resume, connect, run, and dispose', async () => {
  await withCredentialDir(async (dir) => {
    const calls = [];
    const dbPath = require.resolve('../src/db/db');
    const clientPath = require.resolve('../src/services/providers/codesandbox/client');
    ...
    assert.match(calls[2][1], /play-with-docker-keepalive/);
    assert.deepEqual(calls[3], ['dispose']);
    assert.equal(result.success, true);
    assert.equal(result.action, 'keep-alive-sent');
    assert.equal(result.updates.status, 'RUNNING');
    assert.equal(result.updates.metadata.keepAlive.enabled, true);
    assert.equal(result.updates.metadata.keepAlive.lastAction, 'sdk-command');
    assert.match(result.updates.metadata.keepAlive.lastRunAt, /^\d{4}-\d{2}-\d{2}T/);
  });
});
