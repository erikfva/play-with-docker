...
| [`refresh-codespace-http.js`](#refresh-codespace-httpjs) | HTTP-only refresh (no browser) — ~15× faster; list → delete → create → wait for active → stop via direct GitHub Web UI requests |
...
node scripts/codespace-vm.js --credentials /mnt/s3/github/vm-manager123/github.json --action refresh-http --provision-timeout 0
```
...
HTTP-only equivalent of `refresh-codespace.js` — performs the full refresh cycle (list → delete first → create → wait for active → stop) via direct HTTP requests to GitHub's Web UI, with **no browser or Playwright dependency**. ~15× faster than browser automation.

Extracts session cookies from the Playwright `storageState` JSON and makes `fetch` requests directly, parsing HTML with regex for CSRF tokens and codespace data. No PAT or API token required.
...
4. **Wait for active** — polls `GET /codespaces` until the suspend form for the new slug appears in the HTML. Suspend form present = codespace fully active = Docker engine and devcontainer initialized. This step is required: GitHub provisions codespaces asynchronously and stopping too early leaves Docker uninitialized, causing the backend's `initializeSession` to fail.
5. **Stop** the new codespace (`POST /codespaces/{slug}/suspend`), reusing the HTML from the last provision poll to avoid an extra request.
6. **Return** timings, request log, and the new VM info as JSON.

> **Why suspend-form presence?** GitHub does not render status keywords (Active/Stopped) in raw server-rendered HTML. The suspend form (`action="/codespaces/{slug}/suspend"`) is the only reliable active/inactive signal available without a PAT. `api.github.com` returns 401 with session cookies alone. See `ai/LAB-008_codespace-provider/research-lifecycle.md` for full details.
...
| `--provision-timeout <secs>` | `300` | Max seconds to wait for the codespace to become active before stopping. Set to `0` to skip (not recommended — Docker may not be initialized). |
| `--poll-interval <secs>` | `10` | Polling interval for the provision wait. |
...
{
  "ok": true,
  "timings": { "list": 751, "delete": 722, "create": 2353, "provision-wait": 10786, "stop": 988, "total": 16242 },
  "totalMs": 16242,
  "deleted": { "name": "literate fiesta", "slug": "literate-fiesta-…", "status": "deleted" },
  "name": "obscure journey",
  "slug": "obscure-journey-…",
  "url": "https://github.com/codespaces/obscure-journey-…",
  "editorUrl": "https://obscure-journey-….github.dev/",
  "machine": "2-core • 8GB RAM • 32GB",
  "status": "stopped",
  "provisionState": "active",
  "note": "Codespace was fully active (suspend form confirmed) before stop — Docker and devcontainer initialized.",
  "endpoints": [...],
  "requestCount": 8
}
```

`provisionState` is `"active"` when the suspend form was confirmed before stop, or `null` if the provision wait timed out or was disabled. When `--keep-existing` is passed, `deleted` is `null`.
