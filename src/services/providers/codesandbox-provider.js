...
class CodeSandboxProvider extends BaseProvider {
  ...
  async getCredentialStatus(loaded) {
    ...
    if (httpStatus === 401 || httpStatus === 403) {
      return { status: 'INVALID', validated: false, quotas: [], expiresAt: null, limitations: [limitation('status', `getMetaInfo() returned HTTP ${httpStatus}: token is expired, revoked, or lacks required scopes (sandbox_create, vm_manage).`)] };
    }
    if (!metaResult?.data) { const err = new Error(`getMetaInfo() returned HTTP ${httpStatus ?? 'unknown'}`); err.statusCode = httpStatus; throw err; }
    ...
    if (teamId && scraperEnabled) {
      try {
        ...
        const credentialHint = loaded.credentialRef ? path.basename(loaded.credentialRef) : undefined;
        const scraped = await scraper.scrapeCreditsForTeam(teamId, { timeoutMs: 45000, credentialHint });
        if (scraped && (typeof scraped.used === 'number' || typeof scraped.included === 'number')) { creditUsage = scraped.used; creditLimit = scraped.included; creditRemaining = scraped.remaining; creditBillingPeriod = scraped.billingPeriod; creditSource = scraped.url || `https://codesandbox.io/t/usage?workspace=${teamId}`; }
    ...
    return { status: (rateLimitExhausted || creditExhausted) ? 'QUOTA_EXHAUSTED' : 'AVAILABLE', validated: true, quotas, limitations, expiresAt: null, details: { referencePricing: REFERENCE_PRICING, authScopes: meta.auth?.scopes ?? null, referenceLimits: { freePlanConcurrentVmsDefault: 10, includedCreditsDefault: 400, dashboardUrl: teamId ? `https://codesandbox.io/t/usage?workspace=${teamId}` : 'https://codesandbox.io/dashboard' }, ...(creditSource ? { creditSource } : {}), ...(creditBillingPeriod ? { creditBillingPeriod } : {}) } };
  }
